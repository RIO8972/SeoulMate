...
const normPlace = (p, idx = 0) => {
  ...
  const categoryRaw =
    p?.category ?? p?.category_group_name ?? p?.category_name ?? "";
...
export default function CourseCreatePage() {
  ...
  const initialData = useMemo(() => {
    const prefillPlace = state?.prefill?.place
      ? normPlace(state.prefill.place)
      : null;
    return {
      title: "",
      datetime: "",
  ...
  const handleCreate = async (payload) => {
    ...
    try {
      ...
      const body = {
        ...
        places: (payload.selectedPlaces?.length
      ...
      console.log(
        "[CourseCreatePage] /courses response:",
        res.status,
        res.data
      );
